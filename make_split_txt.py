...
def parse_args():
    parser = argparse.ArgumentParser(description='Make split txt for train, val, test')
    parser.add_argument('--input_dir_path', type=str, default="../preprocessed_data_16000_samples",help="input directory path that contain obj files.")
    parser.add_argument('--split_txt_save_dir_path', type=str, default="../preprocessed_data_16000_samples", help="split txt path.")
...
def main(args):
    ...
    f_train = open(os.path.join(args.split_txt_save_dir_path, f"train_list.txt"), 'w')
    f_val = open(os.path.join(args.split_txt_save_dir_path, f"val_list.txt"), 'w')
    f_test = open(os.path.join(args.split_txt_save_dir_path, f"test_list.txt"), 'w')
